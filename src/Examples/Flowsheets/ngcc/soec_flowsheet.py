...
class SoecDesignFlowsheetData(FlowsheetBlockData):
    ...
    def _define_cell_params(self):
        ...
        self.soec.fuel_electrode.tortuosity.fix(5.4)
        ...
        self.soec.oxygen_electrode.porosity.fix(0.35)
        self.soec.oxygen_electrode.tortuosity.fix(3.0)
    ...
    def _add_units(self):
        ...
        self.feed_hx02 = gum.HeatExchanger(
            default={
                "shell":{"property_package":self.o2_side_prop_params},
                "tube":{"property_package":self.steam_prop_params},
            }
    ...
    def _add_arcs(self):
        ...
        self.sweep01b = Arc(
            ...
            destination=self.oxygen_inlet_translator.inlet,
        )
        self.feed01b = Arc(
            doc="Feed heater to SOEC",
            source=self.feed_heater.outlet,
            destination=self.fuel_inlet_translator.inlet,
        )
        self.ostrm03 = Arc(
        ...
        self.feed03 = Arc(
            doc="",
            source=self.feed_hx02.tube_outlet,
            destination=self.feed_hx01.tube_inlet,
        )
        self.ostrm05 = Arc(
            doc="Sweep turbine to feed hx 2",
            source=self.sweep_turbine.outlet,
            destination=self.feed_hx02.shell_inlet,
        )
        self.water01 = Arc(
    ...
    def _add_constraints(self):
        ...
        self.soec_water_utilization = pyo.Var(self.time, initialize=0.7)
        
        @self.Constraint(self.time)
        def soec_water_utilization_eqn(b,t):
            return b.soec_water_utilization[t] == (
    ...
    def _scaling(self):
        ...
        iscale.set_scaling_factor(self.feed_hx02.shell.heat, 1e-6)
        iscale.set_scaling_factor(self.feed_hx02.tube.heat, 1e-6)
        iscale.set_scaling_factor(
            self.feed_hx02.overall_heat_transfer_coefficient[0.0], 1e-2)
        iscale.set_scaling_factor(self.feed_hx02.area, 1e-3)
        ...
        for t in self.time:
            ...
            iscale.set_scaling_factor(
                self.soec.oxygen_inlet.mole_frac_comp[t,"CO2"],1e6)
            iscale.set_scaling_factor(
        ...
        for blk in [self.fuel_inlet_translator,self.fuel_outlet_translator,
            ...
            sx = props.get_default_scaling("mole_frac_comp")
            scale_indexed_constraint(blk.mole_frac_eqn, sx)
            
            sT = props.get_default_scaling("temperature")
            scale_indexed_constraint(blk.temperature_eqn, sT)
    ...
    def _set_initial_inputs(self):
        self.feed_hx02.tube.properties_in[0].pressure.fix(6e5)
        self.feed_hx02.tube.properties_in[0].flow_mol.fix(1200)
        self.feed_hx02.tube.properties_in[0].enth_mol.fix(
        ...
        # self.soec_water_utilization.fix(0.7)
        # Recycle splits
        ...
        self.feed_hx02.area.fix(600)
        self.feed_hx02.overall_heat_transfer_coefficient.fix(100)
    ...
    def initialize(
        ...
        self.sweep_compressor.initialize()
        propagate_state(self.sweep03)
        self.feed_recycle_mix.initialize()
        self.sweep_recycle_mix.initialize()

        propagate_state(self.feed01)
        propagate_state(self.sweep01)
        self.sweep_heater.initialize()
        self.feed_heater.initialize()
        ...
        self.sweep_recycle_split.initialize()
        propagate_state(self.hstrm01)
        self.feed_recycle_split.initialize()
        propagate_state(self.ostrm03)
        self.sweep_hx.initialize()
        propagate_state(self.ostrm04)
        self.sweep_turbine.initialize()
        propagate_state(self.hstrm03)
        propagate_state(self.ostrm05)
        self.feed_hx02.initialize()
        propagate_state(self.feed03)
        self.feed_hx01.initialize()
        propagate_state(self.feed02)
        self.feed_translator.initialize()
        ...
        self.water_heater01.initialize()
        self.water_heater02.initialize()
    ...
    def _add_tags(self):
        ...
        stream_states = tables.stream_states_dict(
            tables.arcs_to_stream_dict(
                ...
                additional={  # streams that are half in HRSG, and may not have arcs
                    ...
                    "ostrm07": self.water_heater02.shell_outlet,
                    "water00": self.water_pump.inlet,
                    "water04": self.water_heater01.tube_outlet,
                    "water05": self.water_heater02.tube_outlet,
        ...
        tag_group["water_utilization"] = iutil.ModelTag(
            ...
            expr=self.soec_water_utilization[0]*100,
            format_string="{:.1f}",
            display_units="%",
        )
