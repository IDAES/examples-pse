# -*- coding: utf-8 -*-
"""
Created on Wed Dec  7 16:10:21 2022
...
def build_model(m):
    ...
    # air_species = ["CO2", "H2O", "N2", "O2", "Ar"]
    # m.fs.air_props = GenericParameterBlock(
    #     **get_prop(air_species, ["Vap"], scaled=True))
...
def set_inputs(m):
    ...
    # *********** TANK INPUTS ***********
    # m.fs.storage_tank.tank_diameter.fix(5)
    # m.fs.storage_tank.tank_length.fix(10)
    ...
    # m.fs.storage_tank.energy_balances.deactivate()
...
def initialize(m, outlvl=idaeslog.NOTSET, solver=None, optarg=None):
    ...
    # m.fs.air_compressor.inlet.flow_mol.unfix()  # kmol/s
...
def build_costing(m, solver=None):
    ...
    nonfuel_resources = ["water treatment chemicals",
                          "desulfur adsorbent",
                          "water",
    ...
    #     return sum(fs.costing.variable_operating_costs[0, j] for j in nonfuel_resources)
    ...
    def total_variable_cost(fs):
        return fs.caes_variable_cost #+ fs.sofc_variable_fuel_cost + fs.sofc_variable_nonfuel_cost
...
def integrate_storage(m):
    ...
    def steam_cycle_heat_duty(fs, t):
        return fs.hrsg_heat_duty[t] - fs.asu_steam_duty[t] + fs.interstage_cooler.heat_duty[0] * 1e-3

    @m.fs.Expression(m.fs.time)
    def net_power(fs, t):
        return fs.gross_power[t] - fs.auxiliary_load[t] - m.fs.air_compressor.control_volume.work[t] * 1e-3
