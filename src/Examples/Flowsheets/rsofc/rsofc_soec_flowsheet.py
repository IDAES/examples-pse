...
def add_soec_air_side_units(fs):
    ...
    fs.air_preheater_1.delta_temperature_in.fix(100)  # fix DT for pinch side
...
def add_soec_unit_2(fs):
    ...
    fs.soec_stack.number_cells.fix(60e6)
...
def _define_cell_params(self):
    self.soec_stack.number_cells.fix(3.222e6)
...
def set_guess(fs):
    ...
    fs.soec_stack.fuel_inlet.flow_mol[0].fix(2600)  # 8000 mol/s (this value initializes), lb is 5400 mol/s
    ...
    fs.soec_stack.oxygen_inlet.flow_mol[0].fix(2600)  # 8000 mol/s (this value initializes), lb is 5400 mol/s
...
def set_inputs(fs):
    ...
    _set_port(  # TODO - replaced mxa1 with air blower inlet
        fs.air_blower.inlet, F=2600, T=288.15, P=1.01325e5,
        comp=air_comp, fix=True
    )
    ...
    fs.aux_boiler_feed_pump.inlet.flow_mol.fix(2600)
...
def initialize_plant(fs, solver):
    ...
    fs.soec_stack.initialize(
        ...
        # solver='ipopt',
        # optarg=optarg,
        current_density_guess=-500,  #mA/cm2
...
def optimize_model(m):
    ...
    m.soec_fs.soec_single_pass_water_conversion
    m.soec_fs.single_pass_water_conversion_constraint = pyo.Constraint(
        expr=1e3*m.soec_fs.soec_single_pass_water_conversion[0]
        >=
        1e3*0.65
    )
    ...
    #     800
    # )
    m.soec_fs.soec_stack.solid_oxide_cell.current_density.setlb(-8000)
    ...
    m.soec_fs.soec_outlet_temperature_constraint = pyo.Constraint(
        expr= abs(m.soec_fs.soec_stack.oxygen_outlet.temperature[0] -
        m.soec_fs.soec_stack.fuel_outlet.temperature[0])
        <=
        40
    )
    m.soec_fs.soec_inlet_temperature_constraint = pyo.Constraint(
        expr= abs(m.soec_fs.soec_stack.oxygen_inlet.temperature[0] -
    ...
    m.soec_fs.soec_fuel_side_DT_constraint = pyo.Constraint(
        expr= abs(m.soec_fs.soec_stack.fuel_outlet.temperature[0] -
        m.soec_fs.soec_stack.fuel_inlet.temperature[0])
        <=
        40
    )
    ...
    # #         float(h)
    # #     )
    # for h in np.linspace(2.5, 0.2, 12):
    #     m.soec_fs.tag_input["hydrogen_product_rate"].fix(
