...
if __name__ == "__main__":
    print('\n\n building methanol flowsheet')
    solver = get_solver()
    m = build_model()  # build flowsheet
    optarg = {'tol': 1e-6,
              'max_iter': 5000}
    solver.options = optarg
    set_inputs()  # unit and stream specifications
    initialize_flowsheet()  # rigorous initialization scheme
    print('DOF before solve: ', degrees_of_freedom(m))
    print('\n\n solving simple methanol flowsheet')
    results = solver.solve(m, tee=True)
    assert results.solver.termination_condition == TerminationCondition.optimal

    print('\n\n adding costing to methanol flowsheet')
    add_costing()  # re-solve with costing equations
    results2 = solver.solve(m, tee=True)
    m.fs.R101.costing.purchase_cost.display()
    print('Initial solution process results:')
    report()  # display initial solution results
    ...
    m.fs.H102.outlet_temp.deactivate()  # optimize outlet in isothermal flash
