...
from pyomo.environ import Constraint, Param, Var, exp, value
...
def make_SOFC_ROM(self):
    ...
    input_index = [i for i in range(n_inputs)]
    input_plus_index = [i for i in range(n_inputs+1)]
    output_index = [i for i in range(n_outputs)]
    samples_index = [i for i in range(n_samples)]
    ...
    # create input vars, these serve as an interface with the user
    ...
    # intialization values to ROM_input
    input_values = [4000, 500, .4, 700, .5, 2.1, .85, .5, 1]

    # create indexed vars
    self.ROM_input = Var(input_index,
                         initialize=build_dict(input_index, input_values))

    self.norm_input = Var(input_index,
                          initialize=0)

    self.F = Var(input_plus_index,
                 initialize=0)
    self.F[0].fix(1)

    self.R = Var(samples_index,
                 initialize=0)

    self.norm_output = Var(output_index,
                           initialize=0)
    ...
    def input_rule(b, i):
        return b.ROM_input[i] == input_dict[i]
...
def initialize_SOFC_ROM(self):
    ...
    def cvfc_wrapper(variable, constraint):
        for i in constraint.keys():
            calculate_variable_from_constraint(
                variable[i], constraint[i])

    # cvfc is giving an error on F, so it needs to be done manually
    def calculate_F():
    ...
    cvfc_wrapper(self.ROM_input, self.input_mapping_eqs)
    cvfc_wrapper(self.norm_input, self.norm_input_eqs)
    calculate_F()
    cvfc_wrapper(self.R, self.R_eqs)
    cvfc_wrapper(self.norm_output, self.norm_output_eqs)
    cvfc_wrapper(self.ROM_output, self.ROM_output_eqs)
    cvfc_wrapper(self.anode_outlet_temperature, self.anode_outlet_eq)
    cvfc_wrapper(self.cathode_outlet_temperature, self.cathode_outlet_eq)
    cvfc_wrapper(self.stack_voltage, self.stack_voltage_eq)
    cvfc_wrapper(self.max_cell_temperature, self.max_cell_temp_eq)
    cvfc_wrapper(self.deltaT_cell, self.deltaT_cell_eq)
